...
md.renderer.rules.heading_open = ((tokens, idx, _o, env) => {
  ...
  const styles: Record<string, string> = {
    ...
    'font-size':
      th.headingSizes[tag as keyof typeof th.headingSizes] ??
      th.headingSizes.h3,
...
md.core.ruler.push('tip_callout', (state) => {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (!m) continue
    const bq =
      i > 0 && tokens[i - 1].type === 'blockquote_open' ? tokens[i - 1] : null
    if (!bq) continue
...
function highlightCached(code: string, lang: string): string | null {
  ...
  if (hlCache.size > HL_CACHE_MAX)
    hlCache.delete(hlCache.keys().next().value as string)
  return out
}
...
md.renderer.rules.em_open = (() =>
  '<em style="font-style: italic;">') as RenderRule
...
function colorTasks(html: string, th: Theme): string {
  return html
    ...
    .replace(
      /☐/g,
      `<span style="color:${th.delColor};font-weight:400">☐</span>`,
    )
}
