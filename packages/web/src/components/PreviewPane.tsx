...
function fitFor(device: PreviewDevice, w: number, h: number): number {
  ...
  return Math.max(
    0.3,
    Math.round(Math.min(1, w / PAD_FRAME.width, h / PAD_FRAME.height) * 1000) /
      1000,
  )
}
...
function offsetForPosition(
  ...
  if (i < 0) {
    ...
    return Math.max(
      0,
      first.top * Math.min(1, Math.max(0, position / first.line)),
    )
  }
  const cur = anchors[i]
...
function buildAnchors(scroll: HTMLElement): Anchor[] {
  ...
  for (const el of scroll.querySelectorAll<HTMLElement>('[data-line]')) {
    ...
    anchors.push({
      line,
      top: (el.getBoundingClientRect().top - box.top) / k + scroll.scrollTop,
    })
  }
  return anchors
}
  ...
  return (
    ...
    >
      <div className="preview-toolbar">
        <div
          className="inline-grid grid-cols-3 bg-[rgba(60,54,44,0.055)] rounded-lg p-0.5 gap-px"
          role="radiogroup"
          aria-label="预览机型"
        >
          {DEVICES.map((d) => {
              ...
              <TooltipHint key={d.id} content={d.name}>
                <button
                  ...
                  className={cn(
                    ...
                    device === d.id &&
                      'bg-[var(--panel-solid)] text-foreground',
                  )}
      ...
      <div className="phone-stage" ref={stageRef}>
        <div
          ...
          style={device === 'pad' ? { zoom: fit } : undefined}
        >
