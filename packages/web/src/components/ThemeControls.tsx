    ...
    return (
      <button
        ...
        className={cn(
          ...
          active &&
            'border-foreground shadow-[0_0_0_1px_var(--foreground),var(--shadow-sm)]',
        )}
        onClick={() => {
      ...
      >
        <span
          className="flex flex-col gap-[3px] pt-[13px] px-3.5 pb-3.5 border-b border-border"
          style={{ background: theme.body.bg ?? '#ffffff' }}
        >
          <span
          ...
          <span
            className="w-7 h-[3px] rounded-[2px] mt-px mb-0.5"
            style={{ background: theme.accent }}
          />
          <span
            className="h-[2px] rounded-[2px] opacity-[0.26]"
            style={{ background: theme.body.color }}
          />
          <span
            className="h-[2px] rounded-[2px] opacity-[0.26] w-[62%]"
            style={{ background: theme.body.color }}
          />
        </span>
        <span
        ...
        <TooltipHint content={`排版主题：${activeTheme.name}`}>
          <DrawerTrigger asChild>
            ...
            >
              ...
              <span
                className="size-[8px] shrink-0 border border-[color-mix(in_oklch,var(--foreground)_16%,transparent)] rounded-full shadow-[inset_0_0_0_1px_rgb(255_255_255/0.35)]"
        ...
        <DrawerPortal>
          ...
          <DrawerContent className="fixed top-0 right-0 bottom-0 left-auto z-[61] w-[min(380px,100vw)] flex flex-col bg-background border-l border-border shadow-[-20px_0_60px_rgb(0_0_0/0.12)] outline-none">
            <div className="flex items-start justify-between gap-4 p-5 border-b border-border">
              <div>
                <DrawerTitle className="m-0 text-base font-[650] leading-[1.3]">
                  排版主题
                </DrawerTitle>
            ...
            <div
              className="flex-1 min-h-0 overflow-y-auto p-5 [&>section+section]:mt-6"
              role="radiogroup"
              aria-label="排版主题"
            >
              <section>
                <h2 className="m-0 mb-2.5 text-xs font-semibold text-muted-foreground">
                  浅色
                </h2>
